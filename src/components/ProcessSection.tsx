...
export default function ProcessSection() {
  return (
    <section id="process" className="py-20 lg:py-24 bg-[#F8FAFC]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {phases.map((item, idx) => (
            <div
              key={idx}
              className="bg-white p-7 rounded-2xl border border-[#E2E8F0] shadow-xs hover:border-[#47C56E]/60 hover:shadow-md transition-all duration-200"
            >
              <span className="text-xs font-bold text-[#00875A] font-mono tracking-wider">
                {item.phase}
              </span>
              <h3 className="text-xl font-bold text-[#091C0F] mt-2 mb-3 font-display">
                {item.title}
              </h3>
